...
cookie_path = "cookies/TK_cookies"
...
def save_cookies(cookies):
    with open(f'{cookie_path}.json', 'w') as file:
        json.dump(cookies, file, indent=4)

def check_expiry(accountname):
    
    with open(f'{cookie_path}_{accountname}.json', 'r') as file:
...
def upload_tiktok(video : str, description : str, accountname  : str, hashtags: list[str]=None,
                   sound_name : str =None, sound_aud_vol : str='mix', schedule : str=None, 
                   day : int=None, copyrightcheck : bool=False, suppressprint : bool=False,
                   headless : bool=True, stealth : bool=False, proxy : dict=None):
    ...
    if cookie_read == False:
        ...
        os.rename(f'{cookie_path}.json',cookie_full_path)

        cookies, cookie_read = read_cookies(cookie_full_path)
        if cookie_read == False:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            sys.exit("ERROR: TIKTOK TOOK TOO LONG TO UPLOAD YOUR FILE (>20min). Try again, if issue persists then try a lower file size or different wifi connection")

        time.sleep(0.2)
        if suppressprint == False:
            print("Tiktok done loading file onto servers")
        ...
        if sound_fail == False:
            ...
            try:
                ...
                time.sleep(2)
                print("POSSIBLE ERROR UPLOADING: Cannot confirm if uploaded successfully, Please check account in a minute or two to confirm.")
            time.sleep(1)
